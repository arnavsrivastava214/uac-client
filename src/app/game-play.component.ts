...
import { FooterComponent } from "./footer/footer.component";
  ...
  const COLORS = [
    ...
    { name: 'green', bg: 'bg-green-500', light: 'bg-green-100', ring: 'ring-green-300' },
    { name: 'blue', bg: 'bg-blue-500', light: 'bg-blue-100', ring: 'ring-blue-300' },
    { name: 'yellow', bg: 'bg-yellow-500', light: 'bg-yellow-100', ring: 'ring-yellow-300' },
    { name: 'pink', bg: 'bg-pink-500', light: 'bg-pink-100', ring: 'ring-pink-300' },
  ];
      ...
      <div class="relative">
        ...
        <div class="bg-white rounded-3xl shadow-2xl p-6 md:p-8">
          ...
          @if (gamePhase() === 'memorize') {
            <div class="text-center mb-8">
              <div class="inline-block">
                <div class="text-4xl mb-3 font-extrabold text-gray-800">👁️ Memorize</div>
          ...
          @if (gamePhase() === 'userTurn') {
            <div class="text-center mb-8">
              <div class="inline-block">
                <div class="text-4xl mb-3 font-extrabold text-gray-800">🎮 Build the Tower</div>
                <p class="text-lg text-gray-600">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            ...
            <div class="relative">
              <div
                class="relative w-full max-w-md mx-auto h-[520px] md:h-[650px] overflow-hidden rounded-3xl bg-gradient-to-b from-gray-50 to-white border border-gray-200 shadow-inner"
              >
                <!-- Memorize Tower (fixed) -->
                @if (gamePhase() === 'memorize') {
                  <div class="absolute inset-0 flex items-end justify-center pb-10">
                    <div class="relative w-64 md:w-80 h-full">
                      @for (block of blocks; track block.index; let i = $index) {
                        ...
                        >
                          <div
                            [class]="
                              'w-64 h-16 md:w-80 md:h-20 rounded-2xl shadow-xl transition-all duration-200 ' +
                              (block.isActive ? 'scale-110 ' + block.color.ring + ' ring-8' : '')
                            "
                            [ngClass]="[block.color.bg, block.isActive ? 'shadow-2xl' : 'shadow-lg']"
                          >
                            <div class="h-full flex items-center justify-center">
                              @if (block.isActive) {
                                <div class="text-white text-4xl animate-pulse">⚡</div>
                              } @else {
                                <div class="text-white opacity-70 text-xl font-semibold">
                                  {{ block.index + 1 }}
                                </div>
                              }
                            </div>
                      ...
                      <!-- Base -->
                      <div class="absolute bottom-2 left-1/2 -translate-x-1/2 w-72 md:w-96 h-4 bg-gray-900 rounded-full shadow-2xl"></div>
                    </div>
                  </div>
                }
                    ...
                    <div class="absolute inset-0 flex items-end justify-center pb-10">
                      <div class="relative w-64 md:w-80 h-full">
                        ...
                        @for (idx of userSequence; track $index; let i = $index) {
                          ...
                          >
                            <div
                              class="w-64 h-16 md:w-80 md:h-20 rounded-2xl shadow-xl"
                ...
                @if (gamePhase() === 'processing') {
                  <div class="absolute inset-0 flex items-center justify-center">
                    <div class="text-center">
                      ...
                      <div class="font-bold text-gray-800 text-xl">Checking your tower…</div>
            ...
            <!-- Palette Area -->
            <div class="relative">
              <div class="bg-gray-50 border border-gray-200 rounded-3xl p-6 shadow-inner">
                <div class="flex items-center justify-between mb-4">
                  <div>
                    <div class="text-xl font-extrabold text-gray-800">🎨 Blocks</div>
                    <div class="text-sm text-gray-500">Tap to add into tower</div>
                  </div>
  
                  <div class="text-sm text-gray-600">
                    <span class="font-bold text-gray-800">{{ userSequence.length }}</span>
                    / {{ getLevelInfo().blocks }}
                  </div>
                </div>
                ...
                <div class="grid grid-cols-5 gap-3 md:gap-4">
                  @for (c of COLORS; track c.name; let i = $index) {
                    <button
                      ...
                      [disabled]="gamePhase() !== 'userTurn' || userSequence.length >= sequence.length"
                ...
                <div class="mt-5 flex items-center justify-between">
                  <div class="text-sm text-gray-600">
                    @if (gamePhase() === 'userTurn') {
                      Build quietly… result will show at the end 😄
                    } @else {
                      Palette locked
                    }
                  </div>
                  ...
                  <button
                    type="button"
                    (click)="undoLast()"
                    [disabled]="gamePhase() !== 'userTurn' || userSequence.length === 0"
              ...
              @if (gamePhase() === 'userTurn') {
                <div class="mt-4 text-xs text-gray-500 text-center">
                  No right/wrong shown now — final score will appear after completion.
                </div>
              }
            </div>
          </div>
          ...
          <div class="mt-10 text-center">
            @if (gamePhase() === 'memorize') {
              <div class="inline-flex items-center gap-2 bg-blue-50 text-blue-700 px-6 py-3 rounded-full">
            ...
            @if (gamePhase() === 'processing') {
              <div class="inline-flex items-center gap-2 bg-purple-50 text-purple-700 px-6 py-3 rounded-full">
  ...
  export class GamePlayComponent implements OnInit, OnDestroy {
    ...
    blocks: Array<{ index: number; color: any; isActive: boolean; userClicked: boolean }> = [];
    sequence: number[] = [];
    userSequence: number[] = [];
