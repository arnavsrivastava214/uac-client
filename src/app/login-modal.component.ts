    ...
    >
      ...
      <div class="w-full max-w-md max-h-[90vh] overflow-hidden">
        ...
        >
          <!-- Header -->
          <div
            class="sticky top-0 bg-white rounded-t-3xl z-10 border-b border-gray-100 px-6 py-5"
          >
            ...
            >
             ...
             flex items-center justify-center mx-auto mb-2 text-sm"
                ...
                <ng-template #forgotSteps>
                  <div class="flex-1 text-center">
                    <div
                      class="w-8 h-8 rounded-full bg-blue-500 text-white font-bold flex items-center justify-center mx-auto mb-2 text-sm"
                    >
                      1
                    </div>
                    <span class="text-xs font-semibold text-blue-600"
                  ...
                  <div class="flex-1 text-center">
                    <div
                      class="w-8 h-8 rounded-full bg-gray-200 text-gray-500 font-bold flex items-center justify-center mx-auto mb-2 text-sm"
                    >
                      2
                    </div>
...
export class LoginModalComponent implements OnInit {
  ...
  async submitForgotPassword(): Promise<void> {
    ...
    try {
      await this.adminPostService
        ...
        .toPromise();
      this.alert.success('Email verified Otp Sent');
      this.isOtpStage.set(true); // ✅ only OTP stage open
    } catch (error) {
      this.alert.error('Student not Found. Please try again.');
    } finally {
      ...
      this.adminPostService
        ...
        .subscribe({
          next: () => {
            ...
            this.alert.success('Registerd Successfully')
  ...
  submitResetPassword() {
    ...
    if (this.newPassword.length < 6) {
      this.alert.error('Password must be at least 6 characters');
      return;
    }
    ...
    this.adminPostService
      .resetPassword(this.forgotPasswordEmail, this.otp, this.newPassword)
      .pipe(finalize(() => this.forgotPasswordLoading.set(false)))
      .subscribe({
        next: () => {
          this.isResetStage.set(false);
          this.isOtpStage.set(false);
          ...
          this.alert.success('Password resert Successfully');
        },
        error: (err) => {
          const msg =
            err?.error?.message ||
  ...
  verifyEmailOtp() {
    ...
    this.adminPostService
      .verifyEmailOtp(this.forgotPasswordEmail, this.otp)
      .pipe(finalize(() => this.forgotPasswordLoading.set(false)))
      .subscribe({
        next: (res: any) => {
